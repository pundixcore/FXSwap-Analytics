...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      `https://fx-swap.io/#/` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0x80b5a32E4F032B2a058b4F29EC95EEfEEB87aDcd' ? 'FX' : token0Address}/${'FX'}`
    )
  } else {
    return (
      `https://fx-swap.io/#/` +
      (remove ? `remove` : `add`) +
      `/${token0Address === '0x80b5a32E4F032B2a058b4F29EC95EEfEEB87aDcd' ? 'FX' : token0Address}/${
        token1Address === '0x80b5a32E4F032B2a058b4F29EC95EEfEEB87aDcd' ? 'FX' : token1Address
      }`
    )
  }
}
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    ...
    return `https://fx-swap.io/#/swap?inputCurrency=${
      token0Address === '0x80b5a32E4F032B2a058b4F29EC95EEfEEB87aDcd' ? 'FX' : token0Address
    }&outputCurrency=${token1Address === '0x80b5a32E4F032B2a058b4F29EC95EEfEEB87aDcd' ? 'FX' : token1Address}`
  }
}
...
export const urls = {
  showTransaction: (tx) => `https://starscan.io/evm/tx/${tx}/`,
  showAddress: (address) => `https://starscan.io/evm/address/${address}/`,
  showToken: (address) => `https://starscan.io/evm/token/${address}/`,
  showBlock: (block) => `https://starscan.io/fxcore/block/${block}/`,
}
